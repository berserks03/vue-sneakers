...
import { type Cart, CartKey, type Item, type Favorite, type Filter } from '../types'



const { cart, addToCart, removeFromCart } = inject(CartKey) as Cart
...
const onClickAddPlus = (item: Item):void => {
    if (!item.isAdded) {
        addToCart(item)
    } else {
        removeFromCart(item)
...
const onChangeSearchInput = debounce((event:Event):void => {
    filters.searchQuery = (event.target as HTMLInputElement).value
}, 300)

const addToFavorite = async (item:Item):Promise<void> => {
    try {
        if (!item.isFavorite) {
            const obj = {
                itemId: item.id
...
const fetchFavorites = async () => {
    try {
        ...
        items.value = items.value.map((item) => {
            const favorite = favorites.find((favorite:Favorite) => favorite.itemId === item.id)
